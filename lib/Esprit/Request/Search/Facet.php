...
class Facet extends \Esprit\Base {
	 ...
	 * Facet filters.
	 *
	 * @var \Esprit\Request_Search_Builder_Filters
	 */
	protected $_filters ;
	 ...
	 *
	 * @return \\Esprit\Request_Search_Facet
	 */
	...
	protected function _data(array $options = array()) {
		...
		if (empty($data['in']) && empty($data['value_field'])) {
			throw new \Esprit\Request\Exception('Facet error : no scope (keys "field","fields","value_field" and "in" are empty)') ;
		}
		...
		if (empty($data['name']) && !empty($data['in'])) {
			...
			throw new \Esprit\Request\Exception('Facet error : the facet\'s name cannot be determined') ;
		}
	 ...
	 *
	 * @param \Esprit\Request_Search_Criteria $facet		Criteria to test.
	 * @return boolean										Yes/no ?
	 */
	public function mergeable(\Esprit\Request\Search\Facet $facet) {
		$data =	$facet->get() ;
	 ...
	 *
	 * @param \Esprit\Request_Search_Criteria $facet		Criteria to merge.
	 * @return \\Esprit\Request_Search_Criteria				This instance (fluid interface).
	 */
	public function merge(\Esprit\Request\Search\Facet $facet) {
