...
abstract class Criteria extends \Esprit\Base {
	...
	protected function _prepare_term($type = 'term') {
		...
		if (!isset($data['in']) || !isset($data['value'])) {
			throw new \Esprit\Request\Exception('Key "in" or "value" empty', $data) ;
		}
	...
	protected function _prepare_missing($type = 'missing') {
		...
		if (!isset($data['in'])) {
			throw new \Esprit\Request\Exception('Key "in" is empty', $data) ;
		}
	...
	protected function _prepare_ids() {
		...
		if (!isset($data['value'])) {
			throw new \Esprit\Request\Exception('Key "value" is empty', $data) ;
		}
	...
	protected function _prepare_range() {
		...
		if (!isset($data['in'])) {
			throw new \Esprit\Request\Exception('Key "in" empty', $data) ;
		}
	...
	protected function _prepare_geo_distance() {
		...
		if (empty($values)) {
			throw new \Esprit\Request\Exception('Keys "values","lat","lon" are empty', $data) ;
		}
	 ...
	 *
	 * @param \Esprit\Request_Search_Criteria $criteria		Criteria to test.
	 * @return boolean										Yes/no ?
	 */
	public function mergeable(\Esprit\Request\Search\Criteria $criteria) {
		$data =	$criteria->get() ;
		foreach($data as $key => $value) {
	 ...
	 *
	 * @param \Esprit\Request_Search_Criteria $criteria		Criteria to merge.
	 * @return \\Esprit\Request_Search_Criteria				This instance (fluid interface).
	 */
	public function merge(\Esprit\Request\Search\Criteria $criteria) {
