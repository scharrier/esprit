 ...
 * @subpackage Request
 */
class Index extends \Esprit\Request {
		...
		if ($this->_body instanceof Set) {
			...
			foreach($iterator as $document) {
				...
				if (empty($doc_content)) {
					throw new \Esprit\Document\Exception('Bulk ' . $this->_action . ' error : empty document in documents set') ;
				}
	...
	/**
	 * Returns the doc in json (here to be override in \Esprit\Request_Uopdate)
	 *
	 * @param  Esprit_Document $document Doc to index
	...
	protected function _response($data) {
		return new \Esprit\Response\Bulk($data, $this->options()) ;
	}
}
