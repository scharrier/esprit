 ...
 * @package	Esprit
 */
class Http extends \Esprit\Transport {
	 ...
	 */
	public function __construct(array $config = null, \Esprit\Factory $factory = null) {
		// Check : curl installed ?
		if (!extension_loaded('curl')) {
			throw new \Esprit\Transport\Exception('Curl is not installed (curl_init function doesn\'t exists).') ;
		}
	 ...
	 *
	 * @return \EspritConnection	Current connection.
	 */
	...
	public function check() {
		$res = $this->call('/') ;
		if (!isset($res)) {
			throw new \Esprit\Transport\Exception('Invalid JSON or empty response') ;
		}
		if (!isset($res['ok']) || (isset($res['ok']) && $res['ok'] !== true)) {
			throw new \Esprit\Transport\Exception('Bad response from ElasticSearch server. Are you sure you\'re calling the good guy ?') ;
		}
	...
	public function call($url = null, $method = 'GET', $data = null) {
		...
		if ($response === false) {
			throw new \Esprit\Transport\Exception(
				'Error during the request (' . curl_errno($this->_connection) . ') : ' .
				curl_error($this->_connection)
			);
		}
		if (!strlen($response)) {
			throw new \Esprit\Transport\Exception('The ES server returned an empty response.') ;
		}

		$return = json_decode($response, true) ;

		if ($return === null) {
			throw new \Esprit\Transport\Exception('Cannot JSON decode the response : ' . $response) ;
		}
