...
class SearchTest extends \PHPUnit_Framework_TestCase {
	...
	public function testFluid() {
		...
		// Break fluid calls
		$request = $this->client->search() ;
		$this->assertTrue($request->instance() instanceof \Esprit\Request\Search) ;

		$request->query() ;
		$this->assertTrue($request->instance() instanceof \Esprit\Request\Search\Builder\Query) ;

		$request->filter() ;
		$this->assertTrue($request->instance() instanceof \Esprit\Request\Search\Builder\Filters) ;

		$request->query() ;
		$this->assertTrue($request->instance() instanceof \Esprit\Request\Search\Builder\Query) ;

		$request->query('test') ;
		$this->assertTrue($request->instance() instanceof \Esprit\Request\Search\Criteria\Query) ;

		$filter = $request->filter(array('value' => 'test', 'in' => 'my_field'))->instance() ;
		$this->assertTrue($filter instanceof \Esprit\Request\Search\Criteria\Filter) ;

		$filter2 = $request->filter(array('value' => 'other test', 'in' => 'my_field'))->instance() ;
		$this->assertTrue($filter2 instanceof \Esprit\Request\Search\Criteria\Filter) ;
